...
A formally verified safety library. Provides safe arithmetic, string
handling, cryptographic primitives, and data validation — all backed by
mathematical proofs in Idris2 with dependent types.
...
* *Idris2* is the sole computation layer. 258 modules, all total, zero
`+believe_me+`.
* *Zig* bridges Idris2’s RefC output to a C ABI shared library
(`+libproven.so+`).
* *Bindings* (120+ languages) are thin FFI wrappers. They never
reimplement logic.
...
Each binding directory (e.g., `+bindings/rust/+`,
`+bindings/rescript/+`) contains a thin wrapper that calls
`+libproven.so+` via FFI. The binding code handles:

[arabic]
. Data marshaling (language types to/from C types)
. Error propagation (mapping C error codes to language exceptions)
. Package metadata (Cargo.toml, package.json, etc.)
...
* Overflow, underflow, and bounds violations are impossible by
construction
...
* *Downstream*: echidna (theorem prover), hypatia (CI/CD), any project
needing verified safety
