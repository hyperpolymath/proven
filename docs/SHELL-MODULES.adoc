...
*Date*: 2026-01-29 *Version*: v1.1.0 (planned) *Status*: Idris2 modules
created, Zig FFI pending
...
*Features*: - Quote handling (single, double, escape sequences) -
Variable expansion safety (`+$VAR+`, `+${VAR}+`) - Command substitution
validation - Injection detection (command chaining, backgrounding, etc.)
...
*Use in valence-shell*: Validate all user input before execution,
...
*Features*: - Named pipe (FIFO) creation via `+mkfifo(2)+` - Anonymous
pipe creation - Path validation (no traversal) - Resource lifecycle
tracking - Deadlock detection
...
*Use in valence-shell*: Process substitution `+<(cmd)+` and `+>(cmd)+`
execution
...
*Features*: - Fork and exec with validation - Process waiting
(blocking/non-blocking) - Exit code handling - Signal delivery - Zombie
reaping - Resource tracking
...
*Use in valence-shell*: External command execution, background jobs,
pipelines
...
*Features*: - Signal handler registration - Signal masking and blocking
- Re-entrancy safety guarantees - Signal set management - Validation
...
*Use in valence-shell*: Ctrl+C handling, SIGCHLD for background jobs,
SIGTSTP for job control
...
*Features*: - TTY detection - Terminal mode management (raw, cooked,
cbreak) - Job control (foreground/background process groups) - Terminal
size detection - Session management
...
*Use in valence-shell*: Interactive mode detection, job control, Ctrl+Z
handling
...
Each module needs Zig FFI implementations: - `+ffi/zig/src/shell.zig+` -
SafeShell FFI - `+ffi/zig/src/pipe.zig+` - SafePipe FFI (mkfifo, pipe) -
...
Each module has proof stubs that need implementation: - *SafeShell*:
Escaped strings contain no unquoted metacharacters - *SafePipe*: FIFO
paths contain no traversal, pipe ends are distinct - *SafeProcess*:
Spawned PIDs are positive, reaped processes not zombies - *SafeSignal*:
SIGKILL/SIGSTOP cannot be caught, mask operations idempotent -
...
*Current Status*: Valence-shell v0.10.0 has shell features in Rust
without formal verification.

*After Integration*: - ✅ Process spawning proven correct (no zombies,
clean exit) - ✅ Signal handling proven safe (no race conditions) - ✅
FIFO management proven leak-free (no orphaned pipes) - ✅ Shell syntax
proven injection-free (no command injection) - ✅ Job control proven
correct (no terminal conflicts)
...
[arabic]
. ✅ Create Idris2 modules (DONE)
. ✅ Update proven.ipkg (DONE)
. ⏳ Build proven package (test compilation)
. ⏳ Create Zig FFI implementations
. ⏳ Test Idris → Zig → Rust integration
. ⏳ Integrate with valence-shell milestones:
