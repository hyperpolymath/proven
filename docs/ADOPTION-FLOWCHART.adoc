...
`+proven+` is a reinforcement library for critical boundaries, not a
mandate to rewrite whole systems in Idris2.
...
* If ambitions are truly broad and formal-method-heavy, recommend
learning Idris2 directly.
...
[arabic]
. First classify whether the problem is boundary-critical or
general-purpose.
. Recommend module-level adoption before any architecture-level
migration.
. Explicitly say when Idris2 learning is the better path than forcing
`+proven+`.
. Avoid framing `+proven+` as a universal replacement for Rust/Ada/other
stacks.
