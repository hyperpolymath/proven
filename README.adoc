...
*Mathematically proven safe operations.* Code that **cannot crash**, verified with Idris2 dependent types and totality checking.
...
image:https://img.shields.io/badge/believe__me-4566_⚠-red[believe_me = 4566 (known technical debt)]
image:https://img.shields.io/badge/overall-55%25-yellow[Overall 55%]

== License

This project is licensed under **PMPL-1.0-or-later**. See link:LICENSE[LICENSE].

WARNING: **proven** is an **Idris2 library with FFI bindings**, not a native implementation in your language. All computation happens in Idris2 with mathematical proofs. Language bindings call this verified code via FFI.

IMPORTANT: **Unbreakable guarantee applies only to Idris2 code.** The Zig layer must be a *pure ABI bridge* with no safety logic. Any non‑Idris logic in bindings is considered non‑proven and must be removed.

== What Makes proven Different?

*Traditional Libraries:* Trust the author got it right +
*proven:* **Mathematical proof** the code cannot crash
...
**Core Implementation:** link:src/Proven/[Idris2 with dependent types] +
**FFI Bridge:** link:ffi/zig/[Zig for C ABI compatibility] (no safety logic) +
**Language Bindings:** Thin wrappers calling verified code only

== Performance Tradeoff

The Idris‑only ABI maximizes safety but introduces FFI overhead. This is a
deliberate tradeoff: **correctness over speed**. Performance improvements will
focus on batching and reducing crossings without re‑introducing non‑proven logic.

== Migration Status

The project is moving all bindings to **pure Zig ABI wrappers** that call Idris
only. Any non‑Idris safety logic in bindings is being removed or rewritten.
...
| *Core Safety*
| <<SafeMath>> • <<SafeString>> • <<SafeFloat>> • <<SafeBuffer>> • <<SafeArgs>> • <<SafeCalculator>>

| *Data Formats*
| <<SafeJson>> • <<SafeXML>> • <<SafeYAML>> • <<SafeTOML>> • <<SafeHtml>> • <<SafeBase64>> • <<SafeHex>> • <<SafeMarkdown>> • <<SafeBibTeX>> • <<SafeArchive>> • <<SafeTemplate>> • <<SafeCSV>>
...
| *System & I/O*
| <<SafeFile>> • <<SafeEnv>> • <<SafeDateTime>> • <<SafeCurrency>> • <<SafeUUID>> • <<SafeLog>> • <<SafeCron>> • <<SafeShell>> • <<SafePipe>> • <<SafeProcess>> • <<SafeSignal>> • <<SafeTerminal>> • <<SafeSemaphore>>
...
| *Mathematics & Physics*
| <<SafeAngle>> • <<SafeProbability>> • <<SafeUnit>> • <<SafeFiniteField>> • <<SafeDecimal>> • <<SafeRational>> • <<SafeComplex>>

| *Distributed Systems*
| <<SafeMonotonic>> • <<SafeRateLimiter>> • <<SafeCircuitBreaker>> • <<SafeRetry>>

| *Data Structures*
| <<SafeQueue>> • <<SafeBloom>> • <<SafeLRU>> • <<SafeSet>> • <<SafeHeap>> • <<SafeMatrix>> • <<SafeBitset>> • <<SafeInterval>> • <<SafeUnionFind>>
...
| Core Modules
| 104
| Idris 2 verified implementations (258 total .idr files including FFI wrappers)

| Binding Targets
| 120+
| Languages, platforms, and paradigms (18 complete, 102 scaffolded)

| believe_me Count
| 0
| Eliminated from ~4,566 instances (remediated 2026-02-22)

| Overall Completion
| ~55%
| Core solid (100%), apps (7%), hardware (0%), convenience (0%)
|===
...
* All 104 core modules have verified implementations (believe_me=0 as of v1.2.0). 120+ binding targets exist (18 fully implemented, 102 scaffolded with FFI stubs). Overall project ~55% complete — see link:TOPOLOGY.md[TOPOLOGY.md] for full dashboard.
...
│                                                                 │
│  • 104 modules with dependent types: compiler PROVES code       │
│    is correct                                                   │
...
* ✅ 104 core modules with dependent type proofs (expanded from 74 in v1.0.0)
* ✅ 14 new FFI exports for Zig bridge layer
* ✅ SPDX headers standardised to PMPL-1.0-or-later across all files
* ✅ Proof fixes: SafeCapability, ECHIDNA/Soundness, SafeFloat
* ✅ SafeCert FFI hostname matching aligned with source module
* ✅ SafeML FFI parsePositive replaced with scalar building blocks
* ✅ ipkg duplicate module entries cleaned up
* ✅ All TODO stubs converted to documentation notes

=== v1.0.0

* ✅ Initial release with 74 core modules and dependent type proofs
* 120+ binding targets via Zig FFI (18 complete, 102 scaffolded)
