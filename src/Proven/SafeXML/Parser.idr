          ...
          in if null (unpack name)
               then Err (InvalidName "" "Empty name")
                 ...
                 case break (== ':') (unpack name) of
                   (pfx, ':' :: local) =>
                     Ok (MkXMLName (pack local) (Just (pack pfx)) Nothing, st')
                   _ =>
       ...
       Just c =>
         if isXMLNameStartChar c
           then do
             ...
             -- REPAIRED 2026-08-27: was `if length [] >= st.options.maxAttributesPerElement`.
             -- `length []` is 0, so the guard reduced to `0 >= n` and could NEVER fire:
             -- the per-element attribute cap was declared but unenforced. `[]` also had
             -- no determined element type, leaving an unsolved hole -- which is why this
             -- module never compiled and never entered any .ipkg. The count is now taken
             -- after recursion, which is the per-element count `maxAttributesPerElement`
             -- names. `>=` on a pre-increment count becomes `>` on the total.
             (attr, st1) <- parseAttribute st'
             (rest, st2) <- assert_total (parseAttributes st1)
             if length (attr :: rest) > st.options.maxAttributesPerElement
               then Err (MalformedXML "Too many attributes")
               else Ok (attr :: rest, st2)
  ...
  where
    ...
    consumeUntil marker st =
      let markerChars = unpack marker
          go : List Char -> List Char -> (String, ParserState)
          go acc inp =
...
parseComment st = do
  ...
  where
    ...
    consumeUntilComment st =
      let go : List Char -> List Char -> (String, ParserState)
          go acc inp =
            case inp of
...
-- NOTE: a ||| docstring cannot precede a mutual block in Idris2; demoted to a comment.
mutual
  ...
  parseElement st = do
    ...
    case peekN 2 st3 of
      ...
      _ => do
        ...
        (children, st6) <- assert_total (parseChildren st5)
        -- Expect closing tag
        st7 <- expect "</" st6
  ...
  parseChildren st =
    ...
    in case peekN 2 st' of
         ...
         ['<', '!'] =>
           case peekN 4 st' of
             ['<', '!', '-', '-'] => do
               ...
               (rest, st2) <- assert_total (parseChildren st1)
               Ok (comment :: rest, st2)
             ['<', '!', '[', 'C'] => do
         ...
         ['<', '?'] => do
           ...
           Ok (pi :: rest, st2)
         ['<', _] => do
           (elem, st1) <- assert_total (parseElement st')
           (rest, st2) <- assert_total (parseChildren st1)
           Ok (elem :: rest, st2)
         _ =>
           if isEOF st'
