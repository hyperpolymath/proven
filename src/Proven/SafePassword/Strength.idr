...
-- covering: windows helper is structurally recursive on list but
-- uses drop which the totality checker cannot verify terminates
...
detectPatterns pwd =
  ...
  where
    ...
    detectKeyboardPattern p =
      let lower = toLower p
      in case Data.List.find (`isInfixOf` lower) keyboardPatterns of
    ...
    detectSequential p =
      let lower = toLower p
          sequences = ["abcd", "bcde", "cdef", "defg", "efgh",
                       ...
                       "dcba", "edcb", "fedc", "gfed"]
      in case Data.List.find (`isInfixOf` lower) sequences of
...
analyzeStrength pwd =
  ...
  where
    ...
    calculatePoolSize cs =
      let hasLower : Bool = any (\c => c >= 'a' && c <= 'z') cs
          hasUpper : Bool = any (\c => c >= 'A' && c <= 'Z') cs
          hasDigit : Bool = any (\c => c >= '0' && c <= '9') cs
          hasSymbol : Bool = any (\c => not (isAlphaNum c) && not (isSpace c)) cs
          base : Double = 0.0
          withLower : Double = if hasLower then base + 26.0 else base
          withUpper : Double = if hasUpper then withLower + 26.0 else withLower
          withDigit : Double = if hasDigit then withUpper + 10.0 else withUpper
          withSymbol : Double = if hasSymbol then withDigit + 32.0 else withDigit
...
quickStrengthCheck pwd = level (analyzeStrength pwd)
...
strengthScore pwd = score (analyzeStrength pwd)
