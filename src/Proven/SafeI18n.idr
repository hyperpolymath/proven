...
||| Replace first occurrence of a substring in a character list
covering
replaceInList : (needle : List Char) -> (replacement : List Char) -> (haystack : List Char) -> List Char
replaceInList _ _ [] = []
replaceInList needle replacement haystack =
  if isPrefixOf needle haystack
    then replacement ++ replaceInList needle replacement (drop (length needle) haystack)
    else case haystack of
           [] => []
           (c :: rest) => c :: replaceInList needle replacement rest
  ...
  where
    ...
    replaceSingle template key value =
      ...
      in if isInfixOf placeholder template
           then pack (replaceInList (unpack placeholder) (unpack value) (unpack template))
           else template
