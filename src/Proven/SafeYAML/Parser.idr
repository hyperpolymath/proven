...
consumeWhile pred st = go [] st.input st
  where
    -- Hoisted 2026-08-27: `go` previously recursed on the ParserState record
    -- alone, so Idris2's size-change checker could not see the input shrink
    -- (record updates are opaque to it). Passing the remaining input as an
    -- explicit second argument makes the structural descent visible. The state
    -- is still threaded for line/col tracking; semantics are unchanged, since
    -- the branch is only taken when `s.input = c :: cs` and `consume` yields
    -- exactly `cs`. Totality DISCHARGED, not budgeted -- no trusted base.
    go : List Char -> List Char -> ParserState -> (String, ParserState)
    go acc [] s = (pack (reverse acc), s)
    go acc (c :: cs) s =
      if pred c
        then let (_, s') = consume s in go (c :: acc) cs s'
...
parseQuotedString quote st =
  ...
  where
    ...
    parseContent acc st = assert_total $
      case consume st of
...
-- Forward declarations: parseFlowSequence/parseFlowMapping and parseValue
-- form a mutually recursive cluster with parseAnchor, parseNumeric,
-- parseAlias and parseTaggedValue. Idris2 resolves top-level
-- names strictly in file order, so the signatures are declared here and
-- defined in place below. (Split decl/def is legal at top level; a `mutual`
-- block would work too but would re-indent ~130 lines for no gain.)
||| Parse any value
...
parseFlowSequence st = do
  ...
  where
    ...
    parseItems acc st = assert_total $
...
parseFlowMapping st = do
  ...
  where
    ...
    parsePairs acc st = assert_total $
      let st' = skipSpaces (skipComment st)
      in case peek st' of
...
-- Parse any value (signature forward-declared above)
parseValue st =
  let st' = skipSpaces (skipComment st)
  in case peek st' of
...
-- Parse numeric value (int or float) (signature forward-declared above)
parseNumeric st =
...
-- Parse anchor definition &name (signature forward-declared above)
parseAnchor st = assert_total $
...
-- Parse alias reference *name (signature forward-declared above)
parseAlias st =
  if not st.options.allowAnchors
...
-- Parse tagged value !tag value (signature forward-declared above)
parseTaggedValue st = assert_total $ do
  case consume st of
    (Just '!', st1) =>
  ...
  case peek st of
    Just '%' =>
      ...
      in if isPrefixOf "YAML" directive
           then let version = trim (strSubstr 4 (cast (minus (length directive) 4)) directive)
