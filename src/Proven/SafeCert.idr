  ...
  where
    ...
    matchWildcard pattern host =
      if isPrefixOf "*." pattern
        then let patLen : Int
                 patLen = cast (Prelude.String.length pattern)
                 suffix : String
                 suffix = strSubstr 2 patLen pattern
                 hostLen : Int
                 hostLen = cast (Prelude.String.length host)
                 suffixLen : Int
                 suffixLen = cast (Prelude.String.length suffix)
                 prefixLen : Int
                 prefixLen = hostLen - suffixLen
                 hostPrefix : String
                 hostPrefix = strSubstr 0 prefixLen host
             in isSuffixOf suffix host && not (isInfixOf "." hostPrefix)
        else pattern == host
...
validateCertificate cert currentTime hostname =
  let c0 : List CertValidity
      c0 = []
      c1 : List CertValidity
      c1 = if isExpiredAt cert currentTime then CertExpired :: c0 else c0
      c2 : List CertValidity
      c2 = if isNotYetValidAt cert currentTime then CertNotYetValid :: c1 else c1
      c3 : List CertValidity
      c3 = if isWeakHash (signatureHash cert) then WeakSignature :: c2 else c2
      c4 : List CertValidity
      c4 = if not (isStrongAlgorithm (keyAlgorithm cert)) then WeakKeyAlgorithm :: c3 else c3
      c5 : List CertValidity
      c5 = if not (matchesHostname cert hostname) then HostnameMismatch hostname :: c4 else c4
  in if null c5 then [CertValid] else c5
