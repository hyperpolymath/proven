...
public export
bitXor : Bitset -> Bitset -> Either BitsetError Bitset
bitXor bs1 bs2 =
  if bs1.capacity /= bs2.capacity
    then Left (CapacityMismatch bs1.capacity bs2.capacity)
...
||| Uses Brian Kernighan's algorithm; not structurally decreasing on Integer.
covering
public export
popCount : Bitset -> Nat
...
||| Get capacity (alias for .capacity record accessor)
public export
getCapacity : Bitset -> Nat
getCapacity bs = bs.capacity

||| Find first set bit (least significant)
||| Recurses on Integer; not structurally decreasing.
covering
public export
...
firstClear bs = firstSet (bitNot bs)
