...
select names qb = do
  refs <- traverse col names
  Ok ({ selectCols := map Right refs } qb)
...
innerJoin tableName tableAlias leftCol rightCol qb = do
  ...
  lc <- maybeToResult (InvalidIdentifier leftCol "Invalid column") (mkIdentifier leftCol)
  rc <- maybeToResult (InvalidIdentifier rightCol "Invalid column") (mkIdentifier rightCol)
...
insertMany tableName colNames rows dialect = do
  tbl <- maybeToResult (InvalidIdentifier tableName "Invalid table name") (mkIdentifier tableName)
  cols <- traverse (\n => maybeToResult (InvalidIdentifier n "Invalid column") (mkIdentifier n)) colNames
  let quotedCols = map (quoteIdentifier dialect) cols
      ...
      makeRow startIdx vals =
        ...
        in "(" ++ joinBy ", " placeholders ++ ")"
      rowStrs = fst (foldl (\(acc, idx), row => (acc ++ [makeRow idx row], idx + colCount)) ([], 0) rows)
      sql = "INSERT INTO " ++ quoteIdentifier dialect tbl ++
...
update tableName setCols qb = do
  tbl <- maybeToResult (InvalidIdentifier tableName "Invalid table name") (mkIdentifier tableName)
  cols <- traverse (\(n, _) => maybeToResult (InvalidIdentifier n "Invalid column") (mkIdentifier n)) setCols
  let setVals = map snd setCols
      ...
      sql = "UPDATE " ++ quoteIdentifier qb.dialect tbl ++
            " SET " ++ joinBy ", " setParts ++ whereClause
  ...
  where
    ...
    renderConditions builder startIdx =
      ...
      joinBy " AND " (map (\_ => "condition") builder.whereConditions)
...
delete tableName qb = do
  ...
  where
    ...
    renderWhereSimple builder =
      joinBy " AND " (zipWith (\_, i => paramPlaceholder builder.dialect i)
  ...
  case qb.fromTable of
    ...
    Just tbl =>
      let selectPart = if null qb.selectCols
                         then "SELECT *"
                         else "SELECT " ++ joinBy ", " (map (renderColRef qb.dialect) qb.selectCols)
          fromPart = " FROM " ++ renderTableRef qb.dialect tbl
          joinPart = if null qb.joins then "" else " " ++ joinBy " " qb.joins
  ...
  where
    ...
    renderWhere builder =
      let conditions = zipWith (\c, i => renderCond builder.dialect c i)
                               builder.whereConditions [0 .. minus (length builder.whereConditions) 1]
      in " WHERE " ++ joinBy " AND " conditions
