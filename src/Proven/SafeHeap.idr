...
getAtIdx : Nat -> List a -> Maybe a
getAtIdx _ [] = Nothing
getAtIdx Z (x :: _) = Just x
getAtIdx (S k) (_ :: xs) = getAtIdx k xs
...
swap i j xs =
  case (getAtIdx i xs, getAtIdx j xs) of
...
bubbleUp i xs =
  let p = parentIndex i
  in case (getAtIdx i xs, getAtIdx p xs) of
       (Just vi, Just vp) =>
         if vi < vp
...
bubbleDown i size xs =
  ...
  where
    ...
    findSmallest curr l r size xs =
      let withLeft = case (getAtIdx curr xs, getAtIdx l xs) of
                       (Just vc, Just vl) => if l < size && vl < vc then l else curr
                       _ => curr
      in case (getAtIdx withLeft xs, getAtIdx r xs) of
...
extractMin (MkHeap (_ :: _) Z) = Nothing  -- Inconsistent state: non-empty list with zero size
...
toSortedList heap = extractAll heap []
  where
    covering
    extractAll : Heap a -> List a -> List a
    extractAll h acc =
