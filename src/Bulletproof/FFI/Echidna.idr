-- SPDX-FileCopyrightText: 2025 Bulletproof Contributors
...
module Bulletproof.FFI.Echidna
...
export
verifyBulletproof : (prover : ProverKind)
                 -> (name : String)
                 -> (statement : String)
                 -> (proof : String)
                 -> IO VerificationResult
verifyBulletproof prover name statement proof = do
...
quickVerify name statement = do
  result <- verifyBulletproof Idris2 name statement ""
  pure result.valid
...
|||     verifyBulletproof Idris2 "addCommutes" "(n m : Nat) -> n + m = m + n" "proof"
|||   case result of
