...
*Date:* 2026-01-24 *Question:* Does Ephapax need separate FFI adapters
for affine vs linear modes? *Answer:* *NO* - One Zig FFI adapter handles
both modes.
...
The difference between affine and linear modes is in *Ephapax’s type
system enforcement*, not in the underlying FFI operations.
...
|*Idris2* |Dependent types prevent buffer overflow, cache overflow,
logic bugs
|===
...
* Difference is in Ephapax’s type annotations (`+LRUCache+` vs
`+LRUCache!+`)
* Same underlying FFI calls in both cases
* Zero runtime overhead
* Gradual migration path (affine → linear)

*This design maximizes code reuse while providing dual safety
guarantees.*
...
[arabic]
. Integrate Zig FFI adapter into Ephapax compiler
. Add `+AffineAPI+` and `+LinearAPI+` to Ephapax standard library
. Test with real VRAM cache implementation
. Verify proofs with ECHIDNA
