 ...
 *
 * Loads libproven via Deno.dlopen and declares all FFI function
 * signatures. This file provides the raw library handle used by
 * every safe_* module. All computation delegates to the formally
 * verified Idris 2 + Zig implementation.
 *
 * @module
 */
 ...
 *
 * @param status - The ProvenStatus integer.
 * @returns A descriptive error message.
 */
export function statusToError(status: number): string {
  switch (status) {
    case ProvenStatus.OK:
      return 'Ok';
    case ProvenStatus.ERR_NULL_POINTER:
      return 'Null pointer';
    case ProvenStatus.ERR_INVALID_ARGUMENT:
      return 'Invalid argument';
    case ProvenStatus.ERR_OVERFLOW:
      return 'Integer overflow';
    case ProvenStatus.ERR_UNDERFLOW:
      return 'Integer underflow';
    case ProvenStatus.ERR_DIVISION_BY_ZERO:
      return 'Division by zero';
    case ProvenStatus.ERR_PARSE_FAILURE:
      return 'Parse failure';
    case ProvenStatus.ERR_VALIDATION_FAILED:
      return 'Validation failed';
    case ProvenStatus.ERR_OUT_OF_BOUNDS:
      return 'Out of bounds';
    case ProvenStatus.ERR_ENCODING_ERROR:
      return 'Encoding error';
    case ProvenStatus.ERR_ALLOCATION_FAILED:
      return 'Allocation failed';
    case ProvenStatus.ERR_NOT_IMPLEMENTED:
...
const DENO_SYMBOLS = {
  ...
  proven_string_is_valid_utf8: {
    ...
    result: { struct: ['i32', 'bool'] },
  },
  proven_string_escape_sql: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'pointer', 'usize'] },
  },
  ...
  proven_header_has_crlf: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  proven_header_is_valid_name: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  proven_header_is_dangerous: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  ...
  proven_header_build_csp: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'pointer', 'usize'] },
  },
  proven_header_build_hsts: {
    parameters: ['i64', 'bool', 'bool'],
    result: { struct: ['i32', 'pointer', 'usize'] },
  },

  // --- SafeCookie (6 functions) ---
  proven_cookie_has_injection: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  proven_cookie_validate_name: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  proven_cookie_validate_value: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  proven_cookie_get_prefix: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'i64'] },
  },
  proven_cookie_build_delete: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'pointer', 'usize'] },
  },

  // --- SafeContentType (3 functions) ---
  proven_content_type_can_sniff_dangerous: {
    parameters: ['pointer', 'usize'],
    result: { struct: ['i32', 'bool'] },
  },
  ...
  proven_probability_or_exclusive: {
    parameters: ['f64', 'f64'],
    result: 'f64',
  },
  ...
  // --- SafePassword (1 function) ---
  proven_password_is_common: {
    parameters: ['pointer', 'usize'],
    result: 'bool',
  },
  ...
  proven_state_machine_create: {
    parameters: ['u32', 'u32'],
    result: 'pointer',
  },
  proven_state_machine_allow: {
    parameters: ['pointer', 'u32', 'u32'],
    result: 'bool',
  },
  proven_state_machine_transition: {
  ...
  proven_tensor_create: {
    parameters: ['usize', 'usize'],
    result: 'pointer',
  },
  proven_tensor_set: {
    parameters: ['pointer', 'usize', 'usize', 'f64'],
    result: 'i32',
  },
  proven_tensor_get: {
...
/**
 * Encode a string to UTF-8 bytes (no null terminator needed for
 * length-prefixed FFI calls).
 *
...
/**
 * Read a C string from a Deno pointer and free it via proven_free_string.
 *
 * @param ptr - Pointer to the C string.
 * @param length - Known byte length of the string.
 * @returns The decoded string, or null if pointer is null.
 */
// deno-lint-ignore no-explicit-any
export function readAndFreeString(ptr: any, length: number): string | null {
  if (!ptr) return null;
...
/**
 * Read a C string from a Deno pointer without freeing it.
 *
 * @param ptr - Pointer to the C string.
 * @param length - Known byte length of the string.
 * @returns The decoded string, or null if pointer is null.
 */
// deno-lint-ignore no-explicit-any
export function readString(ptr: any, length: number): string | null {
  if (!ptr) return null;
  const view = new Deno.UnsafePointerView(ptr);
  const bytes = new Uint8Array(length);
  view.copyInto(bytes);
...
function tryLoad(): boolean {
  ...
  if (typeof Deno !== 'undefined' && typeof Deno.dlopen === 'function') {
    ...
    const paths = customPath ? [customPath, ...LIBRARY_PATHS] : LIBRARY_PATHS;
    ...
    loadError =
      'Failed to load libproven from any known path. Set PROVEN_LIB_PATH env var.';
    return false;
  }
 ...
 *
 * @returns The library symbols object.
 ...
 *
 * @returns True if libproven is loaded and initialized.
 */
 ...
 *
 * @returns Error message or null if loaded successfully.
 */
