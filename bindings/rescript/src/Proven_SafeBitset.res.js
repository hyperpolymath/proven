...
function set(bitset, index) {
  ...
  if (word !== undefined) {
    bitset.words[wordIndex] = word | match[1];
    return {
            TAG: "Ok",
            _0: undefined
          };
...
function unset(bitset, index) {
  ...
  var word = Belt_Array.get(bitset.words, wordIndex);
  if (word !== undefined) {
    bitset.words[wordIndex] = word & PervasivesU.lnot(match[1]);
    return {
            TAG: "Ok",
            _0: undefined
          };
    ...
    if (word !== undefined && word !== 0) {
      for(var bit = 0; bit <= 31; ++bit){
        if (result === undefined) {
          ...
          if (bitIdx < bitset.size && (word & (1 << bit)) !== 0) {
            result = bitIdx;
          }
...
function complement(bitset) {
  for(var i = 0 ,i_finish = bitset.wordCount; i < i_finish; ++i){
    var word = Belt_Array.get(bitset.words, i);
    if (word !== undefined) {
      bitset.words[i] = PervasivesU.lnot(word);
    }
  ...
  if (word$1 !== undefined) {
    bitset.words[bitset.wordCount - 1 | 0] = word$1 & mask;
    return ;
  }
...
function setDifference(bitset, other) {
  ...
  for(var i = 0 ,i_finish = bitset.wordCount; i < i_finish; ++i){
    ...
    var match$1 = Belt_Array.get(other.words, i);
    if (match !== undefined && match$1 !== undefined) {
      bitset.words[i] = match & PervasivesU.lnot(match$1);
    }
...
function isSubsetOf(bitset, other) {
  ...
  for(var i = 0 ,i_finish = bitset.wordCount; i < i_finish; ++i){
    if (result) {
      var match = Belt_Array.get(bitset.words, i);
      var match$1 = Belt_Array.get(other.words, i);
      if (!(match !== undefined && match$1 !== undefined && (match & PervasivesU.lnot(match$1)) === 0)) {
        result = false;
      }
...
function isDisjoint(bitset, other) {
  ...
  for(var i = 0 ,i_finish = bitset.wordCount; i < i_finish; ++i){
    if (result) {
      var match = Belt_Array.get(bitset.words, i);
      var match$1 = Belt_Array.get(other.words, i);
      if (match !== undefined && match$1 !== undefined && (match & match$1) !== 0) {
        result = false;
      }
