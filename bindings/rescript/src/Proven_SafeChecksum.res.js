...
for(var i = 0; i <= 255; ++i){
  ...
  for(var _for = 0; _for <= 7; ++_for){
    c = Proven_Bitwise.land(c, 1) === 1 ? Proven_Bitwise.lxor(Proven_Bitwise.lsr(c, 1), -306674912) : Proven_Bitwise.lsr(c, 1);
  }
  table[i] = c;
}
...
function crc32Bytes(data) {
  ...
  Belt_Array.forEach(data, (function ($$byte) {
          var index = Proven_Bitwise.land(Proven_Bitwise.lxor(crc.contents, $$byte), 255);
          var tableValue = table[index];
          crc.contents = Proven_Bitwise.lxor(Proven_Bitwise.lsr(crc.contents, 8), tableValue);
        }));
  return Proven_Bitwise.lxor(crc.contents, -1);
}
...
function adler32Bytes(data) {
  ...
  return Proven_Bitwise.lor(Proven_Bitwise.lsl(b.contents, 16), a.contents);
}
...
function xorChecksumBytes(data) {
  return Belt_Array.reduce(data, 0, (function (acc, $$byte) {
                return Proven_Bitwise.lxor(acc, $$byte);
              }));
}
...
function xorChecksum(data) {
  ...
  for(var i = 0 ,i_finish = data.length; i < i_finish; ++i){
    ...
    result = Proven_Bitwise.lxor(result, Proven_Bitwise.land(code, 255));
  }
  return result;
}
...
function fletcher16Bytes(data) {
  ...
  return Proven_Bitwise.lor(Proven_Bitwise.lsl(sum2.contents, 8), sum1.contents);
}

function fletcher16(data) {
  var bytes = Belt_Array.makeBy(data.length, (function (i) {
          return Proven_Bitwise.land(data.charCodeAt(i) | 0, 255);
        }));
...
function fletcher32Bytes(data) {
  ...
  return Proven_Bitwise.lor(Proven_Bitwise.lsl(sum2.contents, 16), sum1.contents);
}
...
function sumChecksum(data) {
  ...
  for(var i = 0 ,i_finish = data.length; i < i_finish; ++i){
    var code = data.charCodeAt(i) | 0;
    sum = sum + Proven_Bitwise.land(code, 255) | 0;
  }
  return Proven_Bitwise.land(sum, 255);
}

function twosComplementChecksum(data) {
  var sum = sumChecksum(data);
  return Proven_Bitwise.land(Proven_Bitwise.lnot(sum) + 1 | 0, 255);
}

function verifyChecksum(data, checksum) {
  var sum = sumChecksum(data);
  return Proven_Bitwise.land(sum + checksum | 0, 255) === 0;
}

function toHex(value, widthOpt) {
  var width = widthOpt !== undefined ? widthOpt : 8;
  var result = "";
  var v = Proven_Bitwise.land(value, -1);
  for(var _for = 0; _for < width; ++_for){
    var nibble = Proven_Bitwise.land(v, 15);
    result = "0123456789ABCDEF".charAt(nibble) + result;
    v = Proven_Bitwise.lsr(v, 4);
  }
  return result;
}
