...
function encodeString(input) {
  ...
  for(var i = 0 ,i_finish = input.length; i < i_finish; ++i){
    ...
    if (code <= 255) {
      ...
      var highByte = Proven_Bitwise.lsr(code, 8);
      var highHigh = Proven_Bitwise.lsr(highByte, 4);
      var highLow = Proven_Bitwise.land(highByte, 15);
      result = result + hexChars.charAt(highHigh) + hexChars.charAt(highLow);
      var lowByte = Proven_Bitwise.land(code, 255);
      var lowHigh = Proven_Bitwise.lsr(lowByte, 4);
      var lowLow = Proven_Bitwise.land(lowByte, 15);
...
function constantTimeEqual(hexA, hexB) {
  ...
  for(var i = 0; i < maxLength; ++i){
    ...
    diff = Proven_Bitwise.lor(diff, Proven_Bitwise.lxor(charA, charB));
  }
  if (lengthMatch) {
    return diff === 0;
  } else {
...
function constantTimeEqualBytes(bytesA, bytesB) {
  ...
  for(var i = 0; i < maxLength; ++i){
    ...
    diff = Proven_Bitwise.lor(diff, Proven_Bitwise.lxor(byteA, byteB));
  }
  if (lengthMatch) {
    return diff === 0;
  } else {
...
function xorHex(hexA, hexB) {
  ...
  var result = Belt_Array.mapWithIndex(bytesA, (function (i, byteA) {
          var byteB = bytesB[i];
          return Proven_Bitwise.lxor(byteA, byteB);
        }));
